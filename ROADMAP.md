...
| 11 — Immersive (VR) | parallel w/ 7 | P3 | 📝 OpenXR immersive mode of the Phase 7 Godot 3D world — walk the true GA geometry, no projection collapse (R-0025, Draft). Engine = Godot; delivery = native + web (native-first). Same scene as Phase 7, so the XR rig develops **in parallel**, gated only by the graph binding; only comfort-sensitive VR UX follows scene stability. |
