...
/// `(trace·I − M)` where `M = Σ vᵢvᵢᵀ` is the second-moment matrix of the raw
/// grade-1 coords **about the origin** — NOT the centered covariance. A GA
/// bivector plane passes through the origin (membership measures `v ∧ B`
/// against an origin-plane), so the fit must minimize Σ(vᵢ·n)² about the
/// origin; centering would fit an affine plane through the centroid and hand
/// back a normal that is wrong for every off-center cluster (the seed Math
/// topics scored 0.88 out-of-plane under the centered fit, vs ≤0.35 here).
