...
| [R-0093](0093-lens-bundles.md) +a | 🔬 Lens bundles: a lens (its domain + every plateau, bridge and resource under it) serializes to one portable `lenses/<id>.json` beside the world snapshot — Publish writes it to your sync repo, Adopt lists any repo's `lenses/` and merges a chosen lens onto your map. Content-addressed UUID ids make re-adoption idempotent; nothing of yours is overwritten | POC — Knowledge content | Accepted |
| [R-0096](0096-universitam-lens.md) | 🎓 The owner's actual degree (Licenciatura en Física, plan FÍSICA-2019, Universitam) as its own lens: all 49 asignaturas with clave/cuatrimestre/créditos, the seriación as bridges, and a numbered path in the official order — plus 10 GA/SIA **twins** for cuatrimestres 1–3, each a plateau in its OWN lens joined by an "alternative formulation of" cross-lens bridge, so every topic is studied twice: once to pass, once to understand | POC — Knowledge content | Accepted |
| [R-0097](0097-parallel-view.md) | ⇄ Parallel view: **The Undergraduate** card puts the degree at the front door, and a ⇄ on any twinned topic opens a full-screen two-pane reading — the course you must pass on the left, the same syllabus in GA/SIA on the right. One pair of controls steps BOTH routes, skipping untwinned steps and switching the right lens GA↔SIA as the material demands; stacks vertically on a portrait Boox | POC — Knowledge content | Accepted |
| [R-0098](0098-twins-456.md) | GA & SIA twins for **cuatrimestres 4–6**, taking the ⇄ parallel walk from 10 topics to **20**: vector calculus’ four integral theorems become one, Maxwell’s four equations become `∇F = J`, a boost becomes a rotor, a spinor becomes an even-grade element, and thermodynamics’ differentials become literally legitimate | POC — Knowledge content | Accepted |
| [R-0094](0094-lens-registry.md) | `lenses/index.json` — a lens becomes **discoverable**: publishing keeps an index in step, listing a repo costs **one fetch** and previews each lens’s title and counts, and the bundle is fetched only on Adopt. Rows are untrusted (a path escaping `lenses/` is dropped); repos with no index still list via the folder, honestly labelled | POC — Knowledge content | Accepted |
| [R-0099](0099-path-groups.md) | The whole career fits: a long path (the 49-step degree) folds into **collapsible cuatrimestre sections** — 10 scannable headers with per-section progress, the one you’re on auto-open, continuous numbering, and no nested scroll. Generic + content-derived; every other lens stays flat | POC — Knowledge content | Accepted |
| [R-0100](0100-add-prerequisite.md) | ➕ Add a prerequisite the plan missed: since prereqs come from path ORDER, a real dependency the plan doesn’t sequence (e.g. Geometría Analítica underpins Óptica but is listed after it) never shows — a type-to-find picker lets you add your own, marked + removable, folded into the "Before this, study…" list and its hand-off. Local per-device (like your notes) | POC — Knowledge content | Accepted |
| [R-0101](0101-profile-sync.md) | 🧑 Sync your whole self: all personal state (progress/mastery events, lesson & review state, confusions, added prereqs, adopted lenses, persona, proofs) → `profile/state.json` in your repo, merged additively, auto-pulled/pushed — **secrets excluded by construction**. Plus 🪪 move-my-identity (reveal/import your wizard key) so every device is genuinely the same you | POC — Knowledge content | Accepted |
| [R-0102](0102-move-setup.md) | 📦 Move your whole setup in one paste: one opaque `MPSETUP1.` blob carries the things that can NEVER live in git — wizard key + sync repo + token + model API keys + relays — device-to-device, so a fresh Boox is fully configured by a single paste instead of re-typing every field. Import validates the embedded key; the blob is never pushed, sent, or logged. Complement of R-0101's profile sync (data via git, secrets excluded) | POC — Knowledge content | Accepted |
| [R-0103](0103-which-lens.md) | 🧭 Which lens does it fit?: capturing a topic that isn't on the map (e.g. "reflections") now ranks the LENSES it touches with the matching topics as evidence, and says so honestly — "Mostly Geometric Algebra — also touches Euclidean, vector geometry", or "Spans … — a cross-lens idea" — plus an ask-my-model hand-off carrying your lenses + nearby topics for the conceptual call keyword-matching can't make | POC — Knowledge content | Accepted |
| [R-0104](0104-cross-lens-twins.md) | 🧬 A cross-lens topic becomes linked twins: when the fit verdict names several lenses, tick each one and Capture mints a `"<Lens> view: <name>"` sibling on that lens's own island, bridged to the primary with `alternative formulation of` — the same relation as the seeded GA/SIA pairs, so parallel view walks across them immediately. Ticking a twin homes the primary in the verdict's lens, so no twin is ever a twin of itself | POC — Knowledge content | Accepted |
