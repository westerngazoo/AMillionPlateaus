...
| [SPEC-0044](0044-rhizome-drilldown.md) | Rhizome drill-down: select a term → grow a bridged, drillable child plateau (companion-drafted body, honest offline stub); pure `rhizome.js`; reuses `add_plateau`/`add_bridge` | R-0044 | Accepted |
...
| [SPEC-0046](0046-in-browser-compute-cell.md) | In-browser compute cell: pure `compute.js` + Worker `pyodide-runner.js`; self-hosted lazy Pyodide (numpy/sympy) via the deploy workflow; save a cell as a CRDT-synced "Notebook" resource (`Resource.content` + `ResourceKind::Notebook`) | R-0046 | Accepted |
| [SPEC-0047](0047-installable-pwa.md) | Installable offline PWA: manifest + module `sw.js` over pure `pwa.js` (classify/SWR/shell-refresh/warm pass/`shouldRegister`); icons; zero deps | R-0047 | Accepted |
